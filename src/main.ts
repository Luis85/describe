...
export default class DescribePlugin extends Plugin {
  ...
  override async onload(): Promise<void> {
    ...
    this.registerEvent(this.app.workspace.on('file-menu', (menu, file) => {
      if (!this.active || !canDescribePath(file.path, this.app.vault.configDir)) return;
    ...
    this.addCommand({
      ...
      checkCallback: checking => {
        ...
        if (!this.active || !file || !canDescribePath(file.path, this.app.vault.configDir)) return false;
  ...
  private openDescription(file: TAbstractFile): void {
    if (!this.active) return;
    try {
      const type = extensionKey(toSource(file, this.app.vault.configDir));
      const source = () => {
        // Also checked when a queued save starts and after asynchronous folder creation.
        if (!this.active) throw new Error('Describe was disabled. No new note was created by this save.');
        if (this.app.vault.getAbstractFileByPath(file.path) !== file) throw new Error('The selected item no longer exists.');
        const item = toSource(file, this.app.vault.configDir);
  ...
  private async saveDescription(request: CreateRequest, rememberFolder: string | undefined): Promise<{ path: string; warnings: string[] }> {
    ...
    // Note creation is the commit point. Never invite a duplicate retry after that point.
    if (this.active && rememberFolder !== undefined) {
      try {
        await this.store.remember(key, rememberFolder);
        if (this.active) this.settingsTab.update();
      } catch (error) {
        const detail = error instanceof Error ? error.message : 'Review the destination in settings.';
        warnings.push(`Your note was saved, but the file-type destination could not be remembered. ${detail}`);
      }
    }
